...
/// OpenAI-compatible STT provider using the Audio Transcriptions API
class OpenAISTTProvider final : public STTProvider {
public:
	std::string Transcribe(std::string const& wav_path) override {
		...
		std::string model = OPT_GET("Automation/Speech to Text/Model")->GetString();
		std::string language = OPT_GET("Automation/Speech to Text/Language")->GetString();
		...
		std::string prompt = OPT_GET("Automation/Speech to Text/Prompt")->GetString();
		if (!prompt.empty()) {
			part = curl_mime_addpart(mime);
		...
		if (res != CURLE_OK) {
			LOG_E("stt") << "CURL error: " << curl_easy_strerror(res);
			return "";
		}
...
STTService::STTService(agi::Context *context)
: context(context)
{
	RecreateProvider();
}

STTService::~STTService() = default;

std::atomic<int> STTService::temp_file_counter{0};

void STTService::RecreateProvider() {
	// Currently only one provider; future providers can be selected via configuration
	provider = std::make_unique<OpenAISTTProvider>();
}

void STTService::LoadFromExtradata() {
...
std::string STTService::GetCachedText(AssDialogue const *line) const {
	std::lock_guard<std::mutex> lock(mutex);
...
bool STTService::HasText(AssDialogue const *line) const {
	std::lock_guard<std::mutex> lock(mutex);
	return cache.count(line->Id) > 0;
}

void STTService::Clear() {
	std::lock_guard<std::mutex> lock(mutex);
	cache.clear();
	in_flight.clear();
}

void STTService::StoreInExtradata(AssDialogue *line, std::string const& text) {
...
void STTService::TranscribeAsync(AssDialogue *line, std::function<void(std::string const&)> on_complete) {
	if (!line) return;
	TranscribeAsync(line, line->Start, line->End, std::move(on_complete));
}

void STTService::TranscribeAsync(AssDialogue *line, int start_ms, int end_ms, std::function<void(std::string const&)> on_complete) {
	if (!line) return;
...
void STTService::TranscribeWithLookahead(AssDialogue *line, std::function<void(std::string const&)> on_active_complete) {
	if (!line) return;
	...
	int lookahead = OPT_GET("Automation/Speech to Text/Lookahead Lines")->GetInt();
...
void STTService::InvalidateCache(AssDialogue *line) {
	if (!line) return;
	std::lock_guard<std::mutex> lock(mutex);
	cache.erase(line->Id);
